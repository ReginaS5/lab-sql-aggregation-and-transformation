...
SELECT 
    CONCAT(FLOOR(AVG(film.length) / 60),
            ' hours ',
            AVG(film.length) % 60,
            ' minutes') AS average_duration
FROM
    film;
...
SELECT 
    title, IFNULL(rental_duration, 'NA') AS rental_duration
FROM
    sakila.film
ORDER BY title ASC;
...
SELECT 
    rating, COUNT(*) AS number_of_films_for_each_rating
FROM
    film
GROUP BY rating;
...
SELECT 
    rating, ROUND(AVG(length), 2) AS 'film duration'
FROM
    film
GROUP BY rating
ORDER BY rating ASC;
